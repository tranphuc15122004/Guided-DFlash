...
def remask_second_draft_input(
    ...
    num_draft_tokens = first_draft_ids.shape[1]
    num_tokens_to_remask = min(
        num_draft_tokens,
        max(1, int(round(num_draft_tokens * remask_ratio))),
    )
    remask_indices = torch.randperm(
        num_draft_tokens,
        device=block_output_ids.device,
    )[:num_tokens_to_remask]
    remasked_block_output_ids[:, 1 + remask_indices] = mask_token_id
    return remasked_block_output_ids
...
def dflash_generate(
    ...
    while start < max_length:
        ...
        if block_size > 1:
            ...
            first_draft_logits = target.lm_head(model(
            ...
            remasked_block_output_ids = remask_second_draft_input(
                block_output_ids=block_output_ids,
                first_draft_ids=sample(first_draft_logits),
                mask_token_id=mask_token_id,
            )
            remasked_noise_embedding = target.model.embed_tokens(remasked_block_output_ids)
            second_draft_logits = target.lm_head(model(
                target_hidden=target_hidden,
                noise_embedding=remasked_noise_embedding,
                position_ids=draft_position_ids,
                past_key_values=past_key_values_draft,
                use_cache=True,
                is_causal=False,
