...
def _build_case_record(
    *,
    ...
    vcd_dflash: SimpleNamespace,
    dflash_text: str,
    vcd_text: str,
    avg_kl_divergence: float,
) -> dict[str, Any]:
    dflash_acceptance = [int(x) for x in dflash.acceptance_lengths]
    vcd_acceptance = [int(x) for x in vcd_dflash.acceptance_lengths]
    dflash_mean = _mean(dflash_acceptance)
    vcd_mean = _mean(vcd_acceptance)
    delta = vcd_mean - dflash_mean
    if delta > 1e-9:
        better = "vcd"
    elif delta < -1e-9:
        better = "dflash"
    ...
    return {
        ...
        "vcd_dflash": {
            "num_output_tokens": int(vcd_dflash.num_output_tokens),
            "time_to_first_token": float(vcd_dflash.time_to_first_token),
            "time_per_output_token": float(vcd_dflash.time_per_output_token),
            "acceptance_lengths": vcd_acceptance,
            "acceptance_mean": float(vcd_mean),
            "acceptance_histogram": _acceptance_histogram(vcd_acceptance, block_size),
            "output_text": vcd_text,
        },
        "acceptance_delta_vcd_minus_dflash": float(delta),
    }
...
def _summarize_records(
    ...
    *,
    ...
    vcd_means = [r["vcd_dflash"]["acceptance_mean"] for r in case_records]
    deltas = [r["acceptance_delta_vcd_minus_dflash"] for r in case_records]

    wins_vcd = [r for r in case_records if r["better_method"] == "vcd"]
    ...
    for r in case_records:
        ...
        if key not in by_pattern:
            by_pattern[key] = {"delta": [], "vcd": [], "dflash": []}
        by_pattern[key]["delta"].append(r["acceptance_delta_vcd_minus_dflash"])
        by_pattern[key]["vcd"].append(r["vcd_dflash"]["acceptance_mean"])
        by_pattern[key]["dflash"].append(r["dflash"]["acceptance_mean"])

    pattern_summary = {
        key: {
            "num_cases": len(vals["delta"]),
            "mean_delta_vcd_minus_dflash": _mean(vals["delta"]),
            "mean_vcd_acceptance": _mean(vals["vcd"]),
            "mean_dflash_acceptance": _mean(vals["dflash"]),
        }
        for key, vals in by_pattern.items()
    }

    top_vcd_better = sorted(case_records, key=lambda r: r["acceptance_delta_vcd_minus_dflash"], reverse=True)[:20]
    top_dflash_better = sorted(case_records, key=lambda r: r["acceptance_delta_vcd_minus_dflash"])[:20]
    ...
    return {
        ...
        "overall": {
            ...
            "mean_vcd_acceptance": _mean(vcd_means),
            "mean_delta_vcd_minus_dflash": _mean(deltas),
            "vcd_better_cases": len(wins_vcd),
            "dflash_better_cases": len(wins_dflash),
            "tie_cases": len(ties),
            "vcd_better_ratio": len(wins_vcd) / len(case_records),
            "avg_kl_divergence": _mean(divergence_accumulator),
        },
        "pattern_summary": pattern_summary,
        "top_vcd_better_cases": top_vcd_better,
...
def _save_analysis_artifacts(
    *,
    ...
    with pattern_csv.open("w", encoding="utf-8") as f:
        f.write("pattern,num_cases,mean_delta_vcd_minus_dflash,mean_vcd_acceptance,mean_dflash_acceptance\n")
        for pattern, stats in sorted(summary.get("pattern_summary", {}).items()):
            f.write(
                f"{pattern},{stats['num_cases']},{stats['mean_delta_vcd_minus_dflash']:.6f},"
                f"{stats['mean_vcd_acceptance']:.6f},{stats['mean_dflash_acceptance']:.6f}\n"
            )
...
# generate an answer
# Compare the acceptance length of Dflash and VCD_Dflash in the every step to 
@torch.inference_mode()
def dflash_VCD_generate(
    model: DFlashDraftModel,
    target: AutoModelForCausalLM,
    input_ids: torch.Tensor,
    mask_token_id: int,
    ...
    while start < max_length:
        block_output_ids = output_ids[:, start : start + block_size].clone()
        positive_block_output_ids = output_ids[:, start : start + block_size].clone()
        ...
        if block_size > 1:
            ...
            positive_block_output_ids[: , 1:] = sample(positive_draft_logits , gen=gen)
...
def main() -> None:
    ...
    parser.add_argument("--negative-context-noise-std", type=float, default=0.1)
    ...
    for idx in tqdm(indices, disable=not dist.is_main()):
        ...
        messages_vcd: list[dict[str, str]] = []
        for turn_index, user_content in enumerate(instance["turns"]):
            messages_dflash.append({"role": "user", "content": user_content})
            messages_vcd.append({"role": "user", "content": user_content})
            ...
            vcd_input_text = tokenizer.apply_chat_template(
                messages_vcd,
                tokenize=False,
                add_generation_prompt=True,
                enable_thinking=False,
            ...
            vcd_input_ids = tokenizer.encode(vcd_input_text, return_tensors="pt").to(target.device)
            ...
            vcd_response = dflash_VCD_generate(
                model=draft_model,
                target=target,
                input_ids=vcd_input_ids,
                mask_token_id=draft_model.mask_token_id,
                max_new_tokens=args.max_new_tokens,
                block_size=block_size,
                stop_token_ids=[tokenizer.eos_token_id],
            ...
            vcd_ids = vcd_response.output_ids[0, vcd_response.num_input_tokens:]
            dflash_text = tokenizer.decode(dflash_ids, skip_special_tokens=True)
            vcd_text = tokenizer.decode(vcd_ids, skip_special_tokens=True)

            messages_dflash.append({"role": "assistant", "content": dflash_text})
            messages_vcd.append({"role": "assistant", "content": vcd_text})
    ...
    print(
        ...
        f"VCD-DFLASH={overall.get('mean_vcd_acceptance', 0.0):.3f} | "
        f"Delta={overall.get('mean_delta_vcd_minus_dflash', 0.0):.3f}"
    )
    print(
        "Win ratio | "
        f"VCD better={overall.get('vcd_better_cases', 0)} | "
